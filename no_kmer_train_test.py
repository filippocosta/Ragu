...
def getKmers(sequence, size):
    n = int(len(sequence)/size)*size
    sequence = sequence[0:n]
    return [sequence[x*size:(x+1)*size].upper() for x in range(len(sequence) - size)]
...
for k in kmer:
    ...
    model.add(Bidirectional(LSTM(d))) 
    ...
    f = open("emb_no_k"+str(k)+".txt", "w")
    f.write(str(model.layers[0].get_weights()[0]))
    f.close()
